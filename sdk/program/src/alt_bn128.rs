    ...
    impl PodG1 {
        pub fn decompress(g1_bytes: &[u8]) -> Result<PodG1, AltBn128Error> {
            ...
            decompressed_g1
                ...
                .map_err(|_| AltBn128Error::DecompressingG1Failed)?;
            Ok(PodG1(
                convert_edianness_64(decompressed_g1_bytes.as_slice())
                    .try_into()
...
mod tests {
    ...
    fn alt_bn128_g1_compression() {
        ...
        let g1_le = &*[&change_endianness(&g1_be[0..64]), &[0u8][..]].concat();
        let g1: G1 = G1::deserialize_with_mode(g1_le, Compress::No, Validate::Yes).unwrap();

        // arkworks encodes whether a point is positive or negative in a bitmask,
        // to cover both cases we test negated g1 as well.
        let points = [g1, g1.neg()];

        for point in &points {
            let mut compressed_ref = Vec::new();
            point.serialize_compressed(&mut compressed_ref).unwrap();
            let compressed_ref = change_endianness(&compressed_ref).to_vec();
            let decompressed = PodG1::decompress(&compressed_ref).unwrap();

            assert_eq!(decompressed.0, g1_be);
    ...
    fn alt_bn128_g2_compression() {
        ...
        for point in &points {
            ...
            assert_eq!(decompressed.0, g2_be);
